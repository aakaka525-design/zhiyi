...
040 的延续，收录中低优先级的 UI 问题。不影响核心功能正确性，但影响视觉一致性、代码可维护性和边缘场景下的用户体验。
...
- **J1** SVG querySelector 防御性 null 检查 — 从 040 移入
- **J2** Popup 功能按钮缺少执行确认链路 — 从 040 移入
- **J3** 沉浸模式 getComputedStyle 未缓存 — 从 040 移入
- **J4** transition:all 逐步替换 — 从 040 移入
