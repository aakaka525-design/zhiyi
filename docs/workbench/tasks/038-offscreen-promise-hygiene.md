...
# 038 — `ensureOffscreenDocument()` rejected-promise 卫生修复

## 背景

038 讨论确认：`tts.js` 的 `ensureOffscreenDocument()` 与 036 的 `ensureReady()` 同类问题 — `createDocument()` 失败后 `creatingOffscreen` 保留 rejected promise，后续所有 TTS 音频播放永久失败。
...
注意：
- 使用 `try/finally` 而非 `catch` — 这里没有半初始化对象需要清理（与 036 不同）
